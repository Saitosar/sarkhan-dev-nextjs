...
export async function getProcessedPosts({ locale, page = 1, pageSize = 10 }) {
  const url = `${API_URL}/api/posts?locale=${locale}&populate=cover&fields=title,content,slug,excerpt,likes,publishedAt&sort=publishedAt:desc&pagination[page]=${page}&pagination[pageSize]=${pageSize}`;  
  try {
